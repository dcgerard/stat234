...
\item The definition of a $p$-value is the probability of seeing something \emph{as extremem or more extreme} (under the null) than what we saw.
...
Say we wanted to test the hypothesis that Bob uses clouds more than 50\% of the time. So $H_0: p = 0.5$ vs $H_A: p < 0.5$.
