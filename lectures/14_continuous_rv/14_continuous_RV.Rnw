...
\def\chapnum{Week \#3: Continuous Uniform Distribution}
\title{Title}
\author{Author Name}
\date{Lecture Date}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{document}
%%%%%%%%%%%%%%%%
\begin{frame}[allowframebreaks, fragile]{ Continuous Random Variables}
\vspace{0.2cm}
...
\item
$P(2\le X\le 4)$ = $P(1\le X\le 3)$ = $P(5\le X\le 7)$
\vskip0.2cm
and we say that $X$ has a \textbf{Uniform distribution}\\

\item
...and for each $x$ in $S_X$, $P(X=x)=c>0.$
...
We say that $X$ is a {\bf continuous Uniform random variable}, \\
...
Educated guess: $P(X\le 2) = 2(0.10) = 0.2$
\vskip0.3cm
<<echo=FALSE, fig.width=4.2, fig.height=2.2, out.width='0.69\\linewidth'>>=
a <- 0
b <- 10
...
Educated guess by symmetry and balance: $\mu=E(X)=5$.
\vskip0.4cm
...
My guess was $\sigma=2.5$, \\
just the simple median of possible distances from $\mu=5$.
