...
\item What are some sources of bias (unwanted systematic tendancies in the data collection)?
...
\item ``Using a procedure that is only wron 5\% of the time, we reject the hypothesis that startups are more likely to succeed than fail.''
