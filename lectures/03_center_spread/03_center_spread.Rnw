...
qplot(pv, sadvec, geom = "line", xlab = "x", ylab = "SAD(x)") +
...
qplot(potential_length, sad_vec, geom = "line", ylab = "SAD(w)", xlab = "w") +
  geom_vline(xintercept = m, color = "red") +
  annotate("text", label = "median", x = 114, y = 43480)
@
\vskip-0.25cm
Where is the function $SAD(w)$ smallest (minimized)?
\end{frame}
...
\begin{align*}
  ...
  + (\Sexpr{x[3]} - w)^2
  + (\Sexpr{x[4]} - w)^2\\
  &= (\Sexpr{x[1]^2} - 2\Sexpr{x[1]}w + w^2)
  + \Sexpr{x[2]^2} - 2\Sexpr{x[2]}w + w^2)\\
  &\quad + (\Sexpr{x[3]^2} - 2\Sexpr{x[3]}w + w^2)
  + (\Sexpr{x[4]^2} - 2\Sexpr{x[4]}w + w^2)\\
&= \Sexpr{a}w^2  - \Sexpr{b} w + \Sexpr{c}
\end{align*}
So, as ugly as $\displaystyle{\sum_{i=1}^n (x_i - w)^2}$ originally looks
...
qplot(pv, sdvec, geom = "line", xlab = "w", ylab = "SSD(w)") +
  scale_x_continuous(breaks = x)
@
...
&=\sum (x_i^2 - 2wx_i + w^2)\\
&=\sum x_i^2 - 2w\sum x_i + \sum w^2\\
&=\sum x_i^2 - 2w\sum x_i + n w^2\\
\end{align*}
So
\begin{align*}
\frac{d}{dw}f(w) &= -2\sum x_i + 2nw \overset{set}{=} 0 \Rightarrow w = \frac{1}{n}\sum x_i = \bar{x}
\end{align*}
...
&= \frac{1}{n}\sum_{i = 1}^{n}(a + bx_i) \text{ (definition of $y_i$)}\\
&= \frac{1}{n}\sum_{i = 1}^{n}a + b\frac{1}{n}\sum_{i = 1}^{n}x_i \text{ (associative property)}\\
&= \frac{1}{n}\sum_{i = 1}^{n}a + b\bar{x} \text{ (definition of $\bar{x}$)}\\
&= \frac{1}{n}n a + b\bar{x} \text{ ($n$ summations of $a$)}\\
&= a + b\bar{x} \text{ ($n$'s cancel)}
\end{align*}
\end{frame}
