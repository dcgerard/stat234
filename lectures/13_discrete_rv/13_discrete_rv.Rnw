...
\begin{frame}[fragile]{Mean Intuition IV: Average when simulate a lot of data}
<<>>=
...
Find  expected value (mean) of random variable $Y$.\hfill Is $E(Y)=2?$
...
Proof?\qquad See HW\#4 ...and easier methods next week.
